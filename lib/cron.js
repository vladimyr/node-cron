(function(root, factory) {
	...
	function buildExpression(options) {
		const fields = timeUnits.map(function(timeUnit, index) {
			// Treat `null` values the same as omissions.
			const value = options[timeUnit] !== null ? options[timeUnit] : undefined;
			const defaultValue = CronTime.parseDefaults[index];
			const ranges = castArray(value || defaultValue);
			return ranges.join(',').replace(/\s+/, '');
		});
		return fields.join(' ');
	}
	...
	CronTime.prototype = {
		_init: function() {
			...
			if (this.source !== null && typeof this.source === 'object') {
				this.source = buildExpression(this.source);
			}
